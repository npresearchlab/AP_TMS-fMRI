...
% README
% Format: 
% each subject is a two digit number: sub-xx
% sub-xx folder has anat & func folders
% func .nii file titled : sub-xx_BOLD.nii
% anat .nii file titled : sub-xx_T1.nii
%
%
% For Ashley <3
% sub- and spm12 folder: /Users/nprluser/Documents/AP/Nav_Preproc
...
pacing = input("Type 'c' to continue.\n", 's');
...
% ----------------------- SPM Preprocessing Batch ----------------------- %
    ...
    matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_named_file.files = {{sprintf('%s/%s_BOLD.nii', subject_func, subject)}};
...
% --------------------------- Coregistration --------------------------- %
    ...
    matlabbatch{4}.spm.spatial.coreg.estwrite.source = {sprintf('%s/%s_T1.nii,1', subject_anat, subject)};
...
% ----------------------- SPM Preprocessing Batch ----------------------- %
    ...
    %}
    matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_named_file.name = 'Nav_Preproc';
    matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_named_file.files = {{sprintf('%s_task-flanker_run-1_bold.nii.gz', subject)}};
...
% --------------------------- Coregistration --------------------------- %
    ...
    % Anatomical Reference Image
    % matlabbatch{4}.spm.spatial.coreg.estwrite.source = {sprintf('%s/%s_T1.nii,1', subject_anat, subject)};
    matlabbatch{4}.spm.spatial.coreg.estwrite.source = {sprintf('%s_T1w.nii.gz,1', subject)};
    matlabbatch{4}.spm.spatial.coreg.estwrite.other = {''};
    matlabbatch{4}.spm.spatial.coreg.estwrite.eoptions.cost_fun = 'nmi';
    matlabbatch{4}.spm.spatial.coreg.estwrite.eoptions.sep = [4 2];
    matlabbatch{4}.spm.spatial.coreg.estwrite.eoptions.tol = [0.02 0.02 0.02 0.001 0.001 0.001 0.01 0.01 0.01 0.001 0.001 0.001];
