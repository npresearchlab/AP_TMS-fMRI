...
% Tried to convert CSV to individual columns in .xlsx spreadsheet - decided
% to do this in the Python code instead
%{
csvFile = 'output.csv';
csvContent = fileread(csvFile);
lines = strsplit(csvContent, '\n');
cellData = cell(numel(lines), 1);

for i = 1:numel(lines)
    cellData{i} = strsplit(lines{i}, char(32));
end

filename = 'formattedOutput.xlsx';
combinedData = cellData;

xlswrite(filename, csvContent, 'CSV');
%}
